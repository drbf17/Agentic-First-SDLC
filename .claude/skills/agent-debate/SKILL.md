...
Spawn 5–10 sub-agents (all Sonnet 5), each with the same core question but a different framing.
Each agent returns 5–10 ideas independently — no agent sees what the others produce.
A final synthesizer agent (Opus 5) counts how often each idea appears across agents, ranks by frequency, and flags interesting outliers.
...
Spawn 3–5 sub-agents (Sonnet 5). Run 3–4 rounds.
