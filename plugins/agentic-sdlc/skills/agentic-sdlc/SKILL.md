...
| Sealed spec exists but its branch hasn't been created yet | → perform **"Branching, immediately after the spec seals"** (below) — this is a one-time step, not a per-turn check | continue |
| Sealed spec flagged `architecture_recommendation: requires_review`, and the human wants it addressed before building | → offer **`architecture-judge`** skill. **Optional, human-invoked only** — the orchestrator never dispatches it on its own. | continue |
...
Stage 0/1 Spec intake+judge spec-judge skill          interactive with the PO, adversarial
          Branching                                   orchestrator, immediately after seal — see below
Stage 1.5 Architecture grill architecture-judge skill OPTIONAL, human-invoked only — see below
...
**Stage 1.5 is deliberately not automatic.** `spec-judge` only *recommends* it (via `architecture_recommendation` in the sealed spec — see that skill). A human decides whether to spend the cycle grilling the architecture too. See "Optional architecture review" below.

## Branching, immediately after the spec seals

This is the one execution step the orchestrator itself performs rather than delegating — it's pipeline sequencing (which branch subsequent work lands on), not build work. It runs **as soon as `spec-judge` seals** (`SEAL` verdict), before Stage 2 or Stage 3 start — not later, and not gated on whether the optional Stage 1.5 architecture review happens. This way every handoff artifact from Stage 2 onward (`WireframeSpec`, `ApiContract`, manifests, verdicts, and an architecture-judge run if the human requests one) is committed to version control from the moment it's produced, instead of sitting as untracked files until Stage 3.
...
6. Commit the sealed spec artifact (`<spec_id>.v1.sealed.yaml`, and any prior raw draft) onto this branch as the first commit: `chore(<spec_id>): seal spec v1`, with `Spec-Id:` and the spec's `content_hash` as trailers. This is what "already versioning the handoffs" means in practice — from here on, nothing produced for this spec lives only on disk.
7. Only once the branch is checked out and that commit lands do you proceed to Stage 1.5 (if the human wants it) or dispatch Stage 2/3 below. Every subsequent commit for this spec — including delta-spec re-seals, wireframes, contracts, and verdicts — lands on this one branch, scoped to each agent's `owns` globs.
...
## Optional architecture review (Stage 1.5)

When the sealed spec's `architecture_recommendation.verdict` is `requires_review`, `spec-judge` says so plainly to the PO/Tech Lead at seal time. Nothing downstream blocks on it — Stage 2/3 can start immediately after branching regardless. If the human wants the recommendation acted on, they invoke `architecture-judge` themselves (`/architecture-judge`); you never dispatch it on inferred need. If they skip it, proceed to Stage 2/3 as normal — that is a supported, explicit choice, not a gap.

If a sealed `ArchitectureDecisionRecord` exists for this `spec_id` (from a completed `architecture-judge` run) by the time Stage 3 starts, it becomes an additional pinned reference for `backend-dev` and `frontend-dev`, alongside the sealed spec and the `ApiContract`.
...
- **Interactive interrogations are skills, not subagents.** `config-bootstrap`, `spec-writer`, `spec-judge`, and `architecture-judge` are conversations with a human; a subagent cannot have that back-and-forth.
...
4. **Agents never merge.** They push to the branch created in "Branching, immediately after the spec seals" (`feature/`, `bugfix/`, or `hotfix/<spec_id>`, per `payload.git.workflow`); `develop_branch`/`default_branch` is only ever updated via a pull request, and only a human merges it.
